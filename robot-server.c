...
char line_filtered[600],rx_filter[600],d2str[5][10];
...
int receive_from_send_to_client(int sock){
	...
	printf("[DEBUG] Recieved %d bytes from %s\n",(int)sizeof(rx_buffer),ipv4_addr_str_client);
    printf("[DEBUG] Data Recieved = %s\n",rx_buffer);
    ...
    printf("[DEBUG] %d bytes of data sent = %s\n",(int)sizeof(tx_buffer), tx_buffer);
	return 0;
}
